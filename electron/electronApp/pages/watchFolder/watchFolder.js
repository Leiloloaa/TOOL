...
function setupWatchListeners() {
  ...
  window.electronAPI.onImageProcessStart((event, data) => {
    const orig =
      typeof data?.originalSize === "number"
        ? `（原大小: ${formatBytes(data.originalSize)}）`
        : "";
    addLog(`开始处理图片: ${data.fileName} ${orig}`.trim(), "info");
  });
  ...
  window.electronAPI.onImageProcessComplete((event, data) => {
    const hasSizes =
      typeof data?.originalSize === "number" &&
      typeof data?.optimizedSize === "number";
    if (hasSizes) {
      const orig = formatBytes(data.originalSize);
      const now = formatBytes(data.optimizedSize);
      const saved = formatBytes(Math.max(data.savedBytes || 0, 0));
      const rate =
        typeof data.compressionRate === "number"
          ? `${data.compressionRate}%`
          : "-";
      addLog(
        `图片处理完成: ${data.fileName} （原: ${orig}，现: ${now}，节省: ${saved}，压缩率: ${rate}）`,
        "success"
      );
    } else {
