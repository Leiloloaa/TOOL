...
const QRCodeTool = {
  ...
  async init() {
    ...
    // 初始化时自动获取当前页面 URL（延迟一下确保 popup 完全加载）
    setTimeout(() => {
      this.getCurrentUrl();
    }, 200);
  },
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
          ...
          if (isActive && !this.urlInputEl.value.trim()) {
  ...
  async getCurrentUrl() {
    try {
      ...
      try {
        ...
        window.showStatusMessage(`查询标签页失败: ${queryError.message}`, 'error');
        return;
      }
      ...
      if (!url) {
        ...
        try {
          ...
          // 但我们可以提示用户手动输入
          window.showStatusMessage('无法自动获取当前页面 URL，请手动输入链接', 'error');
          this.urlInputEl.placeholder = '请手动输入链接地址，或点击"获取当前页面"按钮重试';
          this.urlInputEl.focus();
        } catch (e) {
          window.showStatusMessage('无法获取当前页面 URL，请手动输入链接', 'error');
        }
        return;
      }
      ...
      if (url.startsWith('chrome://') || 
        ...
        this.urlInputEl.placeholder = '当前页面不支持生成二维码，请手动输入链接';
  ...
  async generateQRCodeWithAPI(url) {
    ...
    const apiUrl = `https://api.qrserver.com/v1/create-qr-code/?size=256x256&data=${encodeURIComponent(url)}`;
  ...
  clear() {
    ...
    this.qrcodeCanvasEl.innerHTML = '<p class="qrcode-placeholder">二维码将显示在这里</p>';
    if (this.downloadBtn) {
