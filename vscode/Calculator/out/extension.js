...
function activate(context) {
    ...
    context.subscriptions.push(hoverProvider, codeLensProvider, documentChangeListener, keyListener, calculateCommand, replaceCommand);
}
...
class CalculatorProvider {
    ...
    provideHover(document, position, token) {
        ...
        console.log(`悬停检测: 行文本 = "${text}", 光标位置 = ${position.character}`);
        // 查找当前位置附近的数学表达式
    ...
    handleEnterKey(document, position) {
        ...
        if (expression) {
            ...
            if (result !== null) {
                ...
                edit.replace(document.uri, new vscode.Range(position.line - 1, expressionStart, position.line - 1, expressionEnd), result.toString());
                vscode.workspace.applyEdit(edit);
                // 显示替换提示
                vscode.window.showInformationMessage(`表达式已替换: ${expression} → ${result}`);
            }
        }
    }
    showCalculation() {
